...
class Scheduler(
    ...
    def init_deterministic_inference_config(self):
        ...
        # Both entries are prefill knobs (SPLIT_TILE / PREFILL_TRUNCATION), so
        # the prefill half of the pair decides. Reading the base field alone
        # left truncation unset whenever the operator configured only
...
def dispatch_event_loop(scheduler: Scheduler):
    ...
    # The configured PP size, not the live topology: the MLX runner stub never
    # initializes torch.distributed, so the live property asserts here.
